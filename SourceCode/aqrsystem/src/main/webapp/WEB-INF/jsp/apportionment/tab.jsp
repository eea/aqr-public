...
<s:layout-render name="/WEB-INF/jsp/common/layout_with_menu.jsp">
    <s:layout-component name="content">
        <h2 class="sourceapp">${res['source.label']}</h2>

        <security:allowed event="create">
            <s:link class="ftm insert btn sourceapp" beanclass="eu.europa.ec.aqrsystem.action.ApportionmentActionBean" event="create">${res['source.create']}</s:link>

            <cust:jqueryLink cssclass="ftm insertfromfile btn sourceapp" label="${res['source.import']}" title="${res['source.import.title']}" id="import-btn"/>

            <a class="ftm exportall btn plan" onclick="toggle();">${res['source.exportall']}</a>
        </security:allowed>

        <div id="exportalldiv" style="display: none">

            <s:form beanclass="eu.europa.ec.aqrsystem.action.ApportionmentActionBean">
                <br/>
                <fieldset>
                    <legend style="color: #074B8C">${res['source.exportall.multiple']}</legend>

                    <p>${res['source.exportall.dateinterval']}:</p>
                    ...
                    <p>${res['source.exportall.description']}:</p>
                    ...
                    <s:submit name="exportall" style="display: none" id="exportAndClose" value="${res['source.export']}" class="ftm export btn btn_dialog_save" onclick="closeDialog();"/>
