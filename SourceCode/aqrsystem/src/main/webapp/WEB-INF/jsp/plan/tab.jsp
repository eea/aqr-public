...
<s:layout-render name="/WEB-INF/jsp/common/layout_with_menu.jsp">
    <s:layout-component name="content">
        <h2 class="plan">${res['plan.label']}</h2>

        <security:allowed event="create">
            <s:link class="ftm insert btn plan" beanclass="eu.europa.ec.aqrsystem.action.PlanActionBean" event="create">${res['plan.create']}</s:link>
            <cust:jqueryLink cssclass="ftm insertfromfile btn plan" label="${res['plan.import']}" title="${res['plan.import.title']}" id="import-btn"/>
            <a class="ftm exportall btn plan" onclick="toggle();">${res['plan.exportall']}</a>
        </security:allowed>

        <div id="exportalldiv" style="display: none">

            <s:form beanclass="eu.europa.ec.aqrsystem.action.PlanActionBean">
                <br/>
                <fieldset>
                    <legend style="color: #074B8C">${res['plan.exportall.multiple']}</legend>

                    <p>${res['plan.exportall.dateinterval']}:</p>
                    ...
                    <p>${res['plan.exportall.description']}:</p>
