...
<s:layout-render name="/WEB-INF/jsp/common/layout_with_menu.jsp">
    <s:layout-component name="content">
        <h2 class="measures">${res['measure.label.plural']}</h2>

        <security:allowed event="create">
            <s:link class="ftm insert btn measures" beanclass="eu.europa.ec.aqrsystem.action.MeasureActionBean" event="create">${res['measure.create']}</s:link>

            <cust:jqueryLink cssclass="ftm insertfromfile btn measures" label="${res['measure.import']}" title="${res['measure.import.title']}" id="import-btn"/>

            <a class="ftm exportall btn plan" onclick="toggle();">${res['measure.exportall']}</a>
        </security:allowed>

        <div id="exportalldiv" style="display: none">

            <s:form beanclass="eu.europa.ec.aqrsystem.action.MeasureActionBean">
                <br/>
                <fieldset>
                    <legend style="color: #074B8C">${res['measure.exportall.multiple']}</legend>

                    <p>${res['measure.exportall.dateinterval']}:</p>
                    ...
                    <p>${res['measure.exportall.description']}:</p>
                    ...
                    <s:submit name="exportall" style="display: none" id="exportAndClose" value="${res['measure.export']}" class="ftm export btn btn_dialog_save" onclick="closeDialog();"/>
        ...
        <cust:importDialog>
            <jsp:attribute name="label">${res['measure.import']}</jsp:attribute>
